	...
	pg_t &pg) const
{
  ...
  ldout(cct, 1) << "##mydebug: name : " << oid.name << dendl;
  ldout(cct, 1) << "##mydebug: pooid : " << loc.pool << dendl;
  ...
  if (loc.hash >= 0) {
    ps = loc.hash;
    ldout(cct, 1) << "##mydebug: loc.hash > 0 : " << ps << dendl;
  } else {
    if (!loc.key.empty()){
      ps = pool->hash_key(loc.key, loc.nspace);
      ldout(cct, 1) << "##mydebug: !loc.key.empty() : " << ps << dendl;
    }
    else{
			...
			ps_t *ppps) const
{
  ...
  ldout(cct, 1) << "##mydebug: pps = " << pps << dendl;
  unsigned size = pool.get_size();
  ldout(cct, 1) << "##mydebug: size = " << pps << dendl;
  // what crush rule?
  ...
  for(vector<__u32>::iterator i=osd_weight.begin();i!=osd_weight.end();i++){
      ldout(cct, 1) << "##mydebug: osd_weight = " << *i << dendl;
  } 
