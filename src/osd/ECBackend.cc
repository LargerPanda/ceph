...
{
	map<hobject_t,
		ECBackend::read_request_t, hobject_t::BitwiseComparator>
		reads;
	void read(
		ECBackend *ec,
	...
	RecoveryMessages *m)
{
	...
	if (op.after_progress.data_complete && !oneshot)
	{
		...
		m->t.collection_move_rename(
			coll, tobj,
			coll, ghobject_t(op.soid, ghobject_t::NO_GEN, get_parent()->whoami_shard().shard));
	}
...
{
	shard_id_t shard = get_parent()->whoami_shard().shard; //获得自己的shard编号
	...
	{
		int r = 0;
		ECUtil::HashInfoRef hinfo = get_hash_info(i->first); //对object号做hash
		if (!hinfo)
		{
			r = -EIO;
	...
	RecoveryMessages *m)
{
	...
	if (rop.in_progress.empty() || is_complete == rop.complete.size())
	{
		...
		if(osd->finished_op_num == osd->actual_size){
        	...
        	assert(NULL != sdata);
			osd->actual_size = osd->op_wq.get_queue_size() < window_size ? osd->op_wq.get_queue_size():window_size;
			...
			dout(1)<< ": mydebug: saturate finish, current group_size="<<osd->op_wq.get_queue_size()<<dendl;
		}
	...
	bool for_recovery)
{
		 ...
		 ++i)
	{
		...
		for (set<pg_shard_t>::const_iterator j = i->second.need.begin();
			 j != i->second.need.end();  //对该对象的每个shard
			 ++j)
		{
			if (need_attrs)
			{
		...
		for (list<boost::tuple<uint64_t, uint64_t, uint32_t>>::const_iterator j =
				 i->second.to_read.begin();     //这个second是read_request_t，里面的toread是三个数的元组组成的list，j是其中一个元组
			 j != i->second.to_read.end();
			 ++j)
		{
			reslist.push_back(
			...
			pair<uint64_t, uint64_t> chunk_off_len =
				sinfo.aligned_offset_len_to_chunk(make_pair(j->get<0>(), j->get<1>())); //转换offset
			for (set<pg_shard_t>::const_iterator k = i->second.need.begin(); //对于每个shard
				 k != i->second.need.end();
				 ++k)
			{
				//一个shard的message的object编号的toread
				//也就是说一个shard会有多个object？
	...
	}//到这就把所有对象的需要的所有shard的message填充完了

	for (map<pg_shard_t, ECSubRead>::iterator i = messages.begin();
		 i != messages.end();
		 ++i)//对于每个message
	{
		op.in_progress.insert(i->first);//加入正在处理的list
		shard_to_read_map[i->first].insert(op.tid);//ecbackend的成员变量，每个shard的tid
		i->second.tid = tid;
...
void ECBackend::start_write(Op *op)
{
		 ...
		 ++i)
	{
		...
		pg_stat_t stats =
			should_send ? get_info().stats : parent->get_shard_info().find(*i)->second.stats;
